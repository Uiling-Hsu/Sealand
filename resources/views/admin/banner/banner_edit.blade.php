            ...
            <div class="row">
                <div class="col-md-12">
                    <div class="card" style="max-width: 800px">
                        <div class="card-header"><h3>內頁上方 Banner 編輯</h3></div>
                        <div class="card-body">
                            {!! Form::model($banner,['url' => '/admin/banner/'.$banner->id ,'method' => 'PATCH','enctype'=>'multipart/form-data'])  !!}

                                <!-- title Form text Input -->
                                <div class="form-group row">
                                    {!! Form::label('title_tw',"區塊名稱:",['class'=>'col-sm-2 form-control-label']) !!}
                                    <div class="col-sm-10">
                                        {!! Form::text('title_tw',null,['class'=>'form-control','readonly']) !!}
                                ...
                                <!-- title Form text Input -->
                                <div class="form-group row">
                                    {!! Form::label('title_en',"區塊英文名稱:",['class'=>'col-sm-2 form-control-label']) !!}
                                    <div class="col-sm-10">
                                        {!! Form::text('title_en',null,['class'=>'form-control','readonly']) !!}
                                        @if($errors->has('title_en'))
                                            <div class="alert alert-danger " role="alert" style="margin-top:5px">
                                                <i class="fa fa-warning"></i>{{ $errors->first('title_en') }}
                                            </div>
                                        @endif
                                    </div>
                                </div>
                                ...
                                <div class="form-group row">
                                    <label for="imgFile" class="col-sm-2 form-control-label">圖片:(1920x270)</label>
                                    <div class="col-sm-10">
                                        {!! Form::file('imgFile',['class'=>'form-control']); !!}<br>
                                        @if($banner->image)
                                            {{ Html::image($banner->image,null,['style'=>'width:300px;border-radius: 10px']) }}
                                            <div style='padding-top:15px;width:500px;text-align:center'>
                                                <a href="{{ url('/admin/banner/del_img/'.$banner->id ) }}" class="btn btn-danger" onclick='return confirm("是否確定要刪除此圖片?");'>刪除此圖</a>
                                            </div>
                                        @endif
                                    </div>
                                ...
                                <div class="form-group" style="text-align:center">
                                    ...
                                    <a class="btn btn-success" href="{{ url('/admin/banner?bid='.$banner->id ) }}">取消及回上一列表</a>
                                </div>
                            {!! Form::close() !!}
                        </div>
                    </div>
