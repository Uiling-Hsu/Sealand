                    ...
                    <div class="card">
                        <div class="">
                            <div style="padding-top: 20px;padding-left: 10px;padding-right: 10px;font-size: 16px;">
                                <span style="display: inline-block;padding-left: 10px">角色管理 列表</span>
                                <a href="{{ url('/admin/role/create' ) }}" class="btn btn-primary float-md-right">新增 角色管理</a>
                            </div>
                        </div>
                        <div class="card-body">
                            <div class="table-responsive">
                                <table class="table table-striped table-bordered table-hover" id="draggable">
                                    ...
                                    <tr>
                                        ...
                                        <th style="text-align:center">角色名稱</th>
                                        <th style="text-align:center">角色中文標題</th>
                                        <th style="text-align:center">角色中文描述</th>
                                        <th style="text-align:center">所屬權限</th>
                                        <th>狀態</th>
                                        <th>動 作</th>
                                    </tr>
                                    </thead>
                                    <tbody>
                                    <!-- Begin Form -->
                                    {!! Form::open(['url' => '/admin/role/sort']) !!}
                                    @foreach($roles as $key=>$role)
                                        {!! Form::hidden('role_id[]',$role->id) !!}
                                        <tr @if(Request('bid')==$role->id) {!! tableBgColor() !!} @endif class="data" draggable="true">
                                            <td class="id" style="width:5%;text-align:center">{{ $role->id }}</td>
                                            <td class="sort">{{$role->sort}}</td>
                                            <td style="width:15%">{!! $role->slug !!}</td>
                                            <td style="width:15%">{!! $role->name !!}</td>
                                            <td style="width:15%">{!! $role->description !!}</td>
                                            <td style="width:15%">
                                                @if($role->slug=='admin')
                                                    ( 全部功能 )
                                                @else
                                                    @forelse($role->permissions as $key=>$permission)
                                                        <i class="mdi mdi-checkbox-marked mdi-18px"></i> {{$permission->name}}<br>
                                                    @empty
                                                    @endforelse
                                                @endif
                                            </td>
                                            <td style="width: 100px">
                                                <input type="checkbox" class="js-switch" id="status{{$key}}" {{$role->status==1?'checked':''}} />
                                                <script>
                                                    $(function() {
                                                        $('#status{{$key}}').change(function() {
                                                            console.log('Toggle: ' + $(this).prop('checked'));
                                                            $.get('/admin/ajax_switch', {"value": $(this).prop('checked'),"db":'roles',"id":'{{$role->id}}',"field":'status'});
                                                        });
                                                    });
                                                </script>
                                            </td>
                                            <td style="width:15%">
                                                <div class="buttons">
                                                    <a href="{{ url('/admin/role/'.$role->id.'/edit' ) }}" class="btn btn-success">編輯</a>
                                                    <a href="{{ url('/admin/role/'.$role->id.'/delete' ) }}" class="btn btn-danger float-md-right" onclick='return confirm("是否確定要刪除此筆資料?");'>刪除</a>
                                                </div>
                                            </td>
                                        </tr>
                                    @endforeach
...
@section('extra-js')
    ...
    <script>
        document.addEventListener('drop', function(event) {
            ...
            $.get('/admin/ajax_sort', {"data": data,"db":'roles'});
        }, false);
