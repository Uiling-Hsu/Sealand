                ...
                <div class="row align-items-end">
                    <div class="col-lg-8">
                        <div class="page-header-title">
                            ...
                            <div class="d-inline">
                                <h5 style="margin-bottom: 5px;position:relative;top:10px">發證地點代碼</h5>
                                {{--                                <span>各項參數發證地點代碼</span>--}}
                            </div>
                        </div>
                    </div>
                    <div class="col-lg-4">
                        <nav class="breadcrumb-container" aria-label="breadcrumb">
                            <ol class="breadcrumb">
                                ...
                                <li class="breadcrumb-item"><a href="#">發證地點代碼</a></li>
                    ...
                    <div class="card">
                        <div class="">
                            <div style="padding-top: 20px;padding-left: 10px;padding-right: 10px;font-size: 16px;">
                                <span style="display: inline-block;padding-left: 10px">發證地點代碼 列表</span>
                                @if(getAdminUser()->id==1)
                                    <a href="{{ url('/admin/ssite/create' ) }}" class="btn btn-primary float-md-right">新增 發證地點代碼</a>
                                @endif
                            </div>
                        </div>
                        <div class="card-body">
                            <div class="table-responsive">
                                <table class="table table-striped table-bordered" id="draggable">
                                    ...
                                    <tbody>
                                    {!! Form::open(['url' => '/admin/ssite/batch_update'])  !!}
                                    @foreach($ssites as $key=>$ssite)
                                        {!! Form::hidden('ssite_id[]',$ssite->id) !!}
                                        <tr @if(Request('bid')==$ssite->id) {!! tableBgColor() !!} @endif class="data" draggable="true">
                                            <td style="text-align:center;width: 100px;">
                                                {!! Form::checkbox('isdel[]', $ssite->id, null ) !!} 刪除
                                            </td>
                                            <td class="sort">{{$ssite->sort}}</td>
                                            <td class="id">{{$ssite->id}}</td>
                                            <td>
                                                {!! $ssite->title !!}
                                            </td>
                                            @if(getAdminUser()->id==1)
                                                <td>
                                                    <input type="checkbox" class="js-switch" id="status{{$key}}" {{$ssite->status==1?'checked':''}} />
                                                    <script>
                                                        $(function() {
                                                            $('#status{{$key}}').change(function() {
                                                                console.log('Toggle: ' + $(this).prop('checked'));
                                                                $.get('/admin/ajax_switch', {"value": $(this).prop('checked'),"db":'ssites',"id":'{{$ssite->id}}',"field":'status'});
                                                            })
                                                        })
                                                    </script>
                                                </td>
                                            ...
                                            <td>
                                                <div class="buttons">
                                                    <a href="{{ url('/admin/ssite/'.$ssite->id.'/edit' ) }}" class="btn btn-success">編輯</a>
                                                    @if(getAdminUser()->id==1)
                                                        <a href="{{ url('/admin/ssite/'.$ssite->id.'/delete' ) }}" class="btn btn-danger float-md-right" onclick='return confirm("是否確定要刪除此筆資料?\n請注意  ！如果刪除此筆資料，此資料下面的所有內容及段落都將一起刪除，請問是否確認要繼續刪除");'>刪除</a>
                                                    @endif
                                                </div>
                                            </td>
                                        </tr>
                                    @endforeach
                                    @if($ssites && $ssites->count())
                                        <tr>
                                ...
                                {{--<div>&nbsp;</div>
                                <div class="col-md-12">
                                    {{ $ssites->appends(Request::except('page'))->links('vendor.pagination.bootstrap-4') }}
                                </div>--}}
                            </div>
...
@section('extra-js')
    ...
    <script>
        document.addEventListener('drop', function(event) {
            ...
            $.get('/admin/ajax_sort', {"data": data,"db":'ssites'});
        }, false);
