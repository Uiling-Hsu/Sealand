                ...
                <div class="row align-items-end">
                    <div class="col-lg-8">
                        <div class="page-header-title">
                            ...
                            <div class="d-inline">
                                <h5 style="margin-bottom: 5px;position:relative;top:10px">顏色</h5>
                                {{--                                <span>各項參數顏色</span>--}}
                            </div>
                        </div>
                    </div>
                    <div class="col-lg-4">
                        <nav class="breadcrumb-container" aria-label="breadcrumb">
                            <ol class="breadcrumb">
                                ...
                                <li class="breadcrumb-item"><a href="#">顏色</a></li>
                    ...
                    <div class="card">
                        <div class="">
                            <div style="padding-top: 20px;padding-left: 10px;padding-right: 10px;font-size: 16px;">
                                <span style="display: inline-block;padding-left: 10px">顏色 列表</span>
                                <a href="{{ url('/admin/procolor/create' ) }}" class="btn btn-primary float-md-right">新增 顏色</a>
                            </div>
                        </div>
                        <div class="card-body">
                            <div class="table-responsive">
                                <table class="table table-striped table-bordered" id="draggable">
                                    ...
                                    <tbody>
                                    {!! Form::open(['url' => '/admin/procolor/batch_update'])  !!}
                                    @foreach($procolors as $key=>$procolor)
                                        {!! Form::hidden('procolor_id[]',$procolor->id) !!}
                                        <tr @if(Request('bid')==$procolor->id) {!! tableBgColor() !!} @endif class="data" draggable="true">
                                            <td style="text-align:center;width: 100px;">
                                                {!! Form::checkbox('isdel[]', $procolor->id, null ) !!} 刪除
                                            </td>
                                            <td class="sort">{{$procolor->sort}}</td>
                                            <td class="id">{{$procolor->id}}</td>
                                            <td>
                                                {!! $procolor->title !!}
                                            </td>
                                            <td>
                                                <input type="checkbox" class="js-switch" id="status{{$key}}" {{$procolor->status==1?'checked':''}} />
                                                <script>
                                                    $(function() {
                                                        $('#status{{$key}}').change(function() {
                                                            console.log('Toggle: ' + $(this).prop('checked'));
                                                            $.get('/admin/ajax_switch', {"value": $(this).prop('checked'),"db":'procolors',"id":'{{$procolor->id}}',"field":'status'});
                                                        })
                                                    })
                                                </script>
                                            </td>
                                            <td>
                                                <div class="buttons">
                                                    <a href="{{ url('/admin/procolor/'.$procolor->id.'/edit' ) }}" class="btn btn-success">編輯</a>
                                                    <a href="{{ url('/admin/procolor/'.$procolor->id.'/delete' ) }}" class="btn btn-danger float-md-right" onclick='return confirm("是否確定要刪除此筆資料?\n請注意  ！如果刪除此筆資料，此資料下面的所有內容及段落都將一起刪除，請問是否確認要繼續刪除");'>刪除</a>
                                                </div>
                                            </td>
                                        </tr>
                                    @endforeach
                                    @if($procolors && $procolors->count())
                                        <tr>
                                ...
                                <div>&nbsp;</div>
                                <div class="col-md-12">
                                    {{ $procolors->appends(Request::except('page'))->links('vendor.pagination.bootstrap-4') }}
                                </div>
                            </div>
...
@section('extra-js')
    ...
    <script>
        document.addEventListener('drop', function(event) {
            ...
            $.get('/admin/ajax_sort', {"data": data,"db":'procolors'});
        }, false);
