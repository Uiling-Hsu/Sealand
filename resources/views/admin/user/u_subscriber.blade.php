                ...
                <div class="col-md-12">
                    <div class="card">
                        ...
                        <div class="card-body">
                            <div class="row">
                                <div class="col-md-12">
                                    {!! Form::open(['url' => '/admin/u_subscriber','method'=>'GET'])  !!}
                                    {!! Form::hidden('search_has_subscriber',1) !!}
                                    <div class="form-row">
                            ...
                            <div class="table-responsive">
                                <table class="table table-striped table-bordered table-hover">
                                    ...
                                    <tr>
                                        ...
                                        <th style="text-align:center">訂閱記錄</th>
                                        <th>註冊日期 / 更新日期</th>
                                        <th>帳號已啟用</th>
                                        <th>帳號有效</th>
                                    </tr>
                                                ...
                                                <td style="width:40%">
                                                    ...
                                                    @if($subscribers)
                                                        @foreach($subscribers as $subscriber)
                                                            ...
                                                            @endphp
                                                            訂閱 ID：{{$subscriber->id}}<br>
                                                            訂閱日期：{{$subscriber->created_at}}<br>
                                                            最後審核結果：
                                                            @if($subscriber->can_order_car==-1)
                                                                <span style="color: grey">未審核</span>
                                                            @elseif($subscriber->can_order_car==1)
                                                                <span style="color: green">通過</span>
                                                            @else
                                                                <span style="color: red">不通過</span>
                                                            @endif
                                                            <div>訂閱方案：{{$cate?$cate->title:''}}</div>
                                                            @if($subscriber->ret_code)
                                                                <div style="color: {{$subscriber->ret_code=='1000'?'green':'red'}}">API回覆碼：{{$subscriber->ret_code}}</div>
                                                                <div style="color: {{$subscriber->ret_code=='1000'?'green':'red'}}">API訊息：{{$subscriber->ret_message}}</div>
                                                            @endif
                                                            <hr>
                                                        @endforeach
                                                    @endif
