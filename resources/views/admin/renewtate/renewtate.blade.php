                ...
                <div class="row align-items-end">
                    <div class="col-lg-8">
                        <div class="page-header-title">
                            ...
                            <div class="d-inline">
                                <h5 style="margin-bottom: 5px;position:relative;top:10px">訂單續約狀態</h5>
                                {{--                                <span>各項參數訂單續約狀態</span>--}}
                            </div>
                        </div>
                    </div>
                    <div class="col-lg-4">
                        <nav class="breadcrumb-container" aria-label="breadcrumb">
                            <ol class="breadcrumb">
                                ...
                                <li class="breadcrumb-item"><a href="#">訂單續約狀態</a></li>
                    ...
                    <div class="card">
                        <div class="">
                            <div style="padding-top: 20px;padding-left: 10px;padding-right: 10px;font-size: 16px;">
                                <span style="display: inline-block;padding-left: 10px">訂單續約狀態 列表</span>
                                @if(getAdminUser()->id==1)
                                    <a href="{{ url('/admin/renewtate/create' ) }}" class="btn btn-primary float-md-right">新增 訂單續約狀態</a>
                                @endif
                            </div>
                        </div>
                        <div class="card-body">
                            <div class="table-responsive">
                                <table class="table table-striped table-bordered">
                                    ...
                                    <tbody>
                                    {!! Form::open(['url' => '/admin/renewtate/batch_update'])  !!}
                                    @foreach($renewtates as $key=>$renewtate)
                                        {!! Form::hidden('renewtate_id[]',$renewtate->id) !!}
                                        <tr @if(Request('bid')==$renewtate->id) {!! tableBgColor() !!} @endif>
                                            <td style="text-align:center;width: 100px;">
                                                {!! Form::checkbox('isdel[]', $renewtate->id, null ) !!} 刪除
                                            </td>
                                            <td class="id">{{$renewtate->id}}</td>
                                            <td>
                                                {!! $renewtate->ftitle !!}
                                            </td>
                                            <td>
                                            ...
                                            @if(getAdminUser()->id==1)
                                                <td>
                                                    <input type="checkbox" class="js-switch" id="status{{$key}}" {{$renewtate->status==1?'checked':''}} />
                                                    <script>
                                                        $(function() {
                                                            $('#status{{$key}}').change(function() {
                                                                console.log('Toggle: ' + $(this).prop('checked'));
                                                                $.get('/admin/ajax_switch', {"value": $(this).prop('checked'),"db":'renewtates',"id":'{{$renewtate->id}}',"field":'status'});
                                                            })
                                                        })
                                                    </script>
                                                </td>
                                            ...
                                            <td>
                                                <div class="buttons">
                                                    <a href="{{ url('/admin/renewtate/'.$renewtate->id.'/edit' ) }}" class="btn btn-success">編輯</a>
                                                    @if(getAdminUser()->id==1)
                                                        <a href="{{ url('/admin/renewtate/'.$renewtate->id.'/delete' ) }}" class="btn btn-danger float-md-right" onclick='return confirm("是否確定要刪除此筆資料?\n請注意  ！如果刪除此筆資料，此資料下面的所有內容及段落都將一起刪除，請問是否確認要繼續刪除");'>刪除</a>
                                                    @endif
                                                </div>
                                            </td>
                                        </tr>
                                    @endforeach
                                    @if($renewtates && $renewtates->count())
                                        <tr>
                                ...
                                {{--<div>&nbsp;</div>
                                <div class="col-md-12">
                                    {{ $renewtates->appends(Request::except('page'))->links('vendor.pagination.bootstrap-4') }}
                                </div>--}}
                            </div>
