...
class InfoController extends Controller
{
    ...
    public function index(){
        ...
        return view('frontend.index',compact('sliders','dealers','cate13s'));
    }

    function about() {
        $aboutins=Aboutin::whereStatus(1)->orderBy('sort')->get();
        return view('frontend.about',compact('aboutins'));
    }

    function channel() {
        $channelins=Channelin::whereStatus(1)->orderBy('sort')->get();
        return view('frontend.channel',compact('channelins'));
    }

    function internet() {
        $internetins=Internetin::whereStatus(1)->orderBy('sort')->get();
        return view('frontend.internet',compact('internetins'));
    }
    ...
    public function news(){
        ...
        return view('frontend.news',compact('hotins'));
    }
    ...
    public function news_in(Hotin $hotin)
    {
        if(!$hotin)
            abort(404);
        return view('frontend.news_in',compact('hotin'));
    }
    ...
    public function promo(){
        ...
        return view('frontend.promo',compact('promoins'));
    }
    ...
    public function promo_in(Promoin $promoin)
    {
        if(!$promoin)
            abort(404);
        return view('frontend.promo_in',compact('promoin'));
    }
    ...
    public function story(){
        ...
        if(!$storycat_id && $list_storycats) {
            ...
            $storyins=Storyin::where('storycat_id',$storycat_id)->where('status','1')->orderBy('sort')->paginate(6);
        return view('frontend.story',compact('storyins','list_storycats','storycat_id'));
    }
    ...
    public function story_in(Storyin $storyin)
    {
        if(!$storyin)
            abort(404);
        return view('frontend.story_in',compact('storyin'));
    }
    ...
    public function faq(){
        ...
        if(!$faqcat_id && $list_faqcats) {
            ...
            $faqins=Faqin::where('faqcat_id',$faqcat_id)->where('status','1')->orderBy('sort')->get();
        return view('frontend.faq',compact('faqins','list_faqcats','faqcat_id'));
    }
    ...
    public function flow(){
        ...
        if(!$flowcat_id && $list_flowcats) {
            ...
            $flowins=Flowin::where('flowcat_id',$flowcat_id)->where('status','1')->orderBy('sort')->get();
        return view('frontend.flow',compact('flowins','list_flowcats','flowcat_id'));
    }
    ...
    public function contact(){
        ...
        return view('frontend.contact',compact('contact_demand_arr'));
    }
    ...
    public function upload5code($ord_no=''){
        ...
        return view('frontend.upload5code',compact('list_ords','ord'));
    }
	...
	function member_policy() {
		return view('frontend.member_policy');
    }

	function activate_success() {
		return view('frontend.activate_success');
    }
