...
class LoginController extends Controller
{
    ...
    public function getLoginForm()
    {
        ...
        return view('frontend/auth/login',compact('remember_me','email','password'));
        //return view('frontend/auth/login',compact('line_url'));
    }
	...
	public function lineLoginCallBack(Request $request)
	{
		try {
			...
			if($user_profile) {
				...
				return redirect('/#list')->with('modal_success_message','登入成功');
			}
			else{
    ...
    public function authenticate(Request $request)
    {
        ...
        if(!$user){
            return redirect()->intended( route('user.login'))->with('modal_failure_message', '您輸入的帳號或密碼錯誤 !');
        }
        ...
        /*elseif($login_field=='phone'){
            ...
            if(Hash::check($p, $user->password)) {
                auth()->guard('user')->login($user);
                return redirect()->intended('/#list')->with('modal_success_message', '登入成功!');
            }
            else
                dd('not');
        }*/
