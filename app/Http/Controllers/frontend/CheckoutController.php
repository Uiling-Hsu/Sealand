<?php
    ...
    class CheckoutController extends Controller
    {
        ...
        public function creditPaid2(Request $request)
        {
            ...
            $ord->update();
            $user=$ord->user;
            writeLog('刷卡前產生起租款結帳單號','ID:'.$user->id.', 會員姓名：'.$user->name.', 訂單編號：'.$ord->ord_no.', 車輛ID：'.$ord->product_id.', 起租款結帳單號：'.$ord->checkout_no2);
        ...
        public function creditPaid3(Request $request)
        {
            ...
            $ord->update();
            $user=$ord->user;
            writeLog('刷卡前產生迄租款結帳單號','ID:'.$user->id.', 會員姓名：'.$user->name.', 訂單編號：'.$ord->ord_no.', 車輛ID：'.$ord->product_id.', 迄租款結帳單號：'.$ord->checkout_no3);
            ...
            if($ord) {
                if($status == "SUCCESS") {
                    ...
                    if($ord->is_paid3_send_email==0) {
                        ...
                        $ord->update();
                        writeLog('還車應付款繳費成功', '訂單ID：'.$ord->id.' 會員ID：'.$ord->user_id.' 會員姓名：'.$ord->name.', 訂單編號：'.$ord->ord_no.', 車輛ID：'.$ord->product_id.' '.$msg, 0);
                    }
